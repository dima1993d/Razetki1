    ...
    public class RadialSlider : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Update()
        {
            ...
            if (Fans.Length > 0)
            {
                ...
                {
                    Fans[i].transform.Rotate(Vector3.up, CurrentAngle * 360 * Time.deltaTime);
        ...
        private void UpdateRadialImage(float targetAngle)
        {
            ...
            if (targetAngle * 360<330 && targetAngle * 360 > 0)
            {
                Pip.transform.localRotation = Quaternion.Euler(0, 0, -(targetAngle * 360));
            }
            if (Lights.Length>0)
            {
                for (int i = 0; i < Lights.Length; i++)
                {
                    Lights[i].intensity = targetAngle * 2;
                }
            }
            if (Curtains.Length > 0)
            {
                ...
                {
                    Curtains[i].GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, targetAngle * 100);
                }
            }
